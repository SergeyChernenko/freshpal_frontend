<template>
    <div>
        <div id="div_load_r" style="margin-top: 25px; height: 30px" class="vld-parent">
        ...
        <div>
            <div v-for="(mess,index) in all_rec">
        ...
        <div style="display: none;" id="no_rec">
            <p style="margin-top: 100px" class="text_style_2">Нет рекомендаций</p>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            view_update_rating(index){
                for (var i = 0; i < this.all_rec.length; i++){
                    if (this.all_rec[i].username__username == this.rating_result.username){
                        this.all_rec[i].level_user = this.rating_result.level_user
                        this.all_rec[i].positive_user = this.rating_result.positive_user
                    }
                }
                this.all_rec[index].level = this.rating_result.level_p
                this.all_rec[index].positive = this.rating_result.positive_p
                if (this.rating_result.like == true){
                    this.all_rec[index].check_like = true
                    this.all_rec[index].sum_like = this.all_rec[index].sum_like + 1
                }
                if (this.rating_result.like == false){
                    this.all_rec[index].check_like = null
                    this.all_rec[index].sum_like = this.all_rec[index].sum_like - 1
                }
                if (this.rating_result.dislike == true){
                    this.all_rec[index].check_dislike = true
                    this.all_rec[index].sum_dislike = this.all_rec[index].sum_dislike + 1
                }
                if (this.rating_result.dislike == false){
                    this.all_rec[index].check_dislike = null
                    this.all_rec[index].sum_dislike = this.all_rec[index].sum_dislike - 1
                }
            },
            ...
            load_rec(){
                if (this.all_rec.length == 0){
                    document.getElementById("no_rec").style.display = "block"
                    document.getElementById("div_load_r").style.display = "none"
            ...
            get_rec(){
                if (this.stop_get_subs != 0){
                    axios.post(this.$hostname + '/api/home/home/get_recommended/', {
